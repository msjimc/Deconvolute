...
void Engine::AnalyseData()
{
	...
	while (data.size() == 8)
	{
		...
		if (testNames(data[0], data[4]) == true)
		{
			...
			if (indexR1.size() == 3 && indexR2.size() == 3)
			{
		...
		if (NoConsole == true && counter > 9999)
		{
...
map<string, int> Engine::GetCommonBarcodes()
{
	...
	vector<string> data = NextReadPair();
	int count = 0;
	while (data.size() == 8 && count < 100000)//100000
	{
		count++;
		vector<string> indexR1 = PrimerIndexRead(data, 1, false);
		vector<string> indexR2 = PrimerIndexRead(data, 5, false);
...
vector<string> Engine::PrimerIndexRead(vector<string>& data, int line, bool trim)
{
	vector<string> answer;
	string read = data.at(line);
	bool first;
	first = true;
	...
	for (int primerIndex = 0; primerIndex < primers.size(); primerIndex++)
	{
		...
		{
			if (InString_Wobble(read.substr(index, p.length()), p, 0.92f) == true)
			{
				answer.push_back(p);
...
void Engine::SaveTheData(vector<string> data, string key, string FirstPrimer, string SecondPrimer)
{
	...
	{
		string f = "@" + data[1].substr(0, 6) + "-" + data[1].substr(data[1].length() - 6, 6);
